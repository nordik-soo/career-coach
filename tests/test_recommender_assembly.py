...
# ===========================================================================
# Slice 2 (2026-06-23): Layer B target-NOC family filter
# ===========================================================================
...
def test_filter_falls_back_to_minor_group_when_no_exact():
    ...
    postings (first 4 digits match). For target 14200 (accounting
    clerk), 14201 (banking clerk) shares minor group '1420'."""
    matches = [
...
def test_filter_returns_empty_when_no_exact_and_no_minor_group():
    ...
    Layer B's helper will then return empty evidence and the
    fallback prose narrates honestly."""
    matches = [
        _MockMatch(noc_code="13110"),
        _MockMatch(noc_code="14404"),
...
def test_filter_skips_matches_with_no_noc_code():
    """Postings without a resolved NOC can't be matched to family;
    they're dropped from the filtered set."""
    matches = [
...
def test_filter_skips_matches_with_invalid_noc_code_length():
    """A posting with a 4-digit or 6-digit NOC code is filtered out
    -- the minor-group prefix match requires 5-digit codes."""
    matches = [
...
def test_filter_accepts_any_iterable():
    """Helper materializes the input iterable -- generator and tuple
    both work, not just list."""
    matches_tuple = (
        _MockMatch(noc_code="14200"),
        _MockMatch(noc_code="13110"),
    )
...
def test_filter_preserves_match_order():
    """First-seen order preserved -- callers may have ranking
    intent in the input order (engine returns top-N by score)."""
    matches = [
